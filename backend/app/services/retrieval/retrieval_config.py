"""Config loader for Phase 2/3 retrieval settings."""
...
def load_retrieval_runtime_config(
    ...
    weights = RerankWeights(
        visual=_float_env("RETRIEVAL_WEIGHT_VISUAL", weights_raw.get("visual"), RerankWeights.visual),
        caption=_float_env("RETRIEVAL_WEIGHT_CAPTION", weights_raw.get("caption"), RerankWeights.caption),
        ocr=_float_env("RETRIEVAL_WEIGHT_OCR", weights_raw.get("ocr"), RerankWeights.ocr),
        objects=_float_env("RETRIEVAL_WEIGHT_OBJECTS", weights_raw.get("objects"), RerankWeights.objects),
        temporal=_float_env("RETRIEVAL_WEIGHT_TEMPORAL", weights_raw.get("temporal"), RerankWeights.temporal),
    )
    text_index = TextIndexConfig(
        path=Path(os.getenv("RETRIEVAL_TEXT_INDEX_PATH") or text_raw.get("path") or TextIndexConfig.path),
        default_top_k=_int_env(
...
def _read_simple_yaml(path: Path) -> dict[str, Any]:
    """Read the small project YAML subset used by configs/retrieval.yaml."""
    root: dict[str, Any] = {}
